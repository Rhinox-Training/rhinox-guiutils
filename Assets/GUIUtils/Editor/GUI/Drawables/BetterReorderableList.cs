...
namespace Rhinox.GUIUtils.Editor
{
  ...
  {
    public BetterReorderableList.HeaderCallbackDelegate drawHeaderCallback;
    public BetterReorderableList.FooterCallbackDelegate drawFooterCallback = (BetterReorderableList.FooterCallbackDelegate) null;
    ...
    public BetterReorderableList.ElementCallbackDelegate drawElementBackgroundCallback;
    public BetterReorderableList.DrawNoneElementCallback drawNoneElementCallback = (BetterReorderableList.DrawNoneElementCallback) null;
    ...
    public BetterReorderableList.AddCallbackDelegate onAddCallback;
    public BetterReorderableList.AddDropdownCallbackDelegate onAddDropdownCallback;
    ...
    private int id = -1;
    private static BetterReorderableList.Defaults s_Defaults;
    ...
    public BetterReorderableList(IList elements, System.Type elementType) => this.InitList((SerializedObject) null, (SerializedProperty) null, elements, true, true, true, true);
    ...
    public BetterReorderableList(SerializedObject serializedObject, SerializedProperty elements) => this.InitList(serializedObject, elements, (IList) null, true, true, true, true);
    ...
    private float GetElementHeight(int index) => this.elementHeightCallback == null ? this.elementHeight : this.elementHeightCallback(index);

    private Rect GetRowRect(int index, Rect listRect) => new Rect(listRect.x, listRect.y + this.GetElementYOffset(index), listRect.width, this.GetElementHeight(index));
    ...
    private float GetListElementHeight()
    {
      ...
      int count = drawElementCountCallback != null ? drawElementCountCallback.Invoke() : this.count;
      if (count == 0)
        return this.elementHeight + num;
      return this.elementHeightCallback != null ? this.GetElementYOffset(count - 1) + this.GetElementHeight(count - 1) + num : this.elementHeight * (float) count + num;
    }
    ...
    private void DoListElements(Rect listRect)
    {
      ...
      if ((this.m_Elements != null && this.m_Elements.isArray || this.m_ElementList != null) && count > 0)
      {
        if (this.IsDragging() && Event.current.type == UnityEngine.EventType.Repaint)
        {
          ...
          for (int index = 0; index < this.m_NonDragTargetIndices.Count; ++index)
          {
            if (this.m_NonDragTargetIndices[index] != -1)
            {
              ...
              if (this.drawElementBackgroundCallback == null)
                BetterReorderableList.s_Defaults.DrawElementBackground(rect1, index, false, false, this.m_Draggable);
              else
                this.drawElementBackgroundCallback(rect1, index, false, false);
              BetterReorderableList.s_Defaults.DrawElementDraggingHandle(rect1, index, false, false, this.m_Draggable);
              Rect contentRect = this.GetContentRect(rect1);
              if (this.drawElementCallback == null)
              {
                if (this.m_Elements != null)
                  BetterReorderableList.s_Defaults.DrawElement(contentRect, this.m_Elements.GetArrayElementAtIndex(this.m_NonDragTargetIndices[index]), (object) null, false, false, this.m_Draggable);
                else
                  BetterReorderableList.s_Defaults.DrawElement(contentRect, (SerializedProperty) null, this.m_ElementList[this.m_NonDragTargetIndices[index]], false, false, this.m_Draggable);
              }
              else
          ...
          if (this.drawElementBackgroundCallback == null)
            BetterReorderableList.s_Defaults.DrawElementBackground(rect1, this.m_ActiveElement, true, true, this.m_Draggable);
          else
            this.drawElementBackgroundCallback(rect1, this.m_ActiveElement, true, true);
          BetterReorderableList.s_Defaults.DrawElementDraggingHandle(rect1, this.m_ActiveElement, true, true, this.m_Draggable);
          Rect contentRect1 = this.GetContentRect(rect1);
          if (this.drawElementCallback == null)
          {
            if (this.m_Elements != null)
              BetterReorderableList.s_Defaults.DrawElement(contentRect1, this.m_Elements.GetArrayElementAtIndex(this.m_ActiveElement), (object) null, true, true, this.m_Draggable);
            else
              BetterReorderableList.s_Defaults.DrawElement(contentRect1, (SerializedProperty) null, this.m_ElementList[this.m_ActiveElement], true, true, this.m_Draggable);
          }
          else
        ...
        else
        {
          for (int index = 0; index < count; ++index)
          {
            ...
            if (this.drawElementBackgroundCallback == null)
              BetterReorderableList.s_Defaults.DrawElementBackground(rect1, index, flag1, flag2, this.m_Draggable);
            else
              this.drawElementBackgroundCallback(rect1, index, flag1, flag2);
            BetterReorderableList.s_Defaults.DrawElementDraggingHandle(rect1, index, flag1, flag2, this.m_Draggable);
            Rect contentRect = this.GetContentRect(rect1);
            if (this.drawElementCallback == null)
            {
              if (this.m_Elements != null)
                BetterReorderableList.s_Defaults.DrawElement(contentRect, this.m_Elements.GetArrayElementAtIndex(index), (object) null, flag1, flag2, this.m_Draggable);
              else
                BetterReorderableList.s_Defaults.DrawElement(contentRect, (SerializedProperty) null, this.m_ElementList[index], flag1, flag2, this.m_Draggable);
            }
            else
      ...
      else
      {
        rect1.y = listRect.y;
        if (this.drawElementBackgroundCallback == null)
          BetterReorderableList.s_Defaults.DrawElementBackground(rect1, -1, false, false, false);
        else
    ...
    private void DoListHeader(Rect headerRect)
    {
      ...
      else
      {
        if (!this.m_DisplayHeader)
          return;
        BetterReorderableList.s_Defaults.DrawHeader(headerRect, this.m_SerializedObject, this.m_Elements, this.m_ElementList);
      }
    }
    ...
    private void DoListFooter(Rect footerRect)
    {
      ...
      else
      {
        if (!this.displayAdd && !this.displayRemove)
          return;
        BetterReorderableList.s_Defaults.DrawFooter(footerRect, this);
      }
    }
    ...
    private void DoDraggingAndSelection(Rect listRect)
    {
      ...
      switch (current.GetTypeForControl(this.id))
      {
        ...
        case UnityEngine.EventType.KeyDown:
          ...
          this.m_ActiveElement = Mathf.Clamp(this.m_ActiveElement, 0, this.m_Elements != null ? this.m_Elements.arraySize - 1 : this.m_ElementList.Count - 1);
          break;
      }
    ...
    private void UpdateDraggedY(Rect listRect) => this.m_DraggedY = Mathf.Clamp(Event.current.mousePosition.y - listRect.y, this.m_DragOffset, listRect.height - (this.GetElementHeight(this.m_ActiveElement) - this.m_DragOffset));
    ...
    public bool HasKeyboardControl() => GUIUtility.keyboardControl == this.id && CustomEditorGUI.HasCurrentWindowKeyFocus();
    ...
    public class Defaults
    {
      ...
      public void DrawFooter(Rect rect, BetterReorderableList list)
      {
      ...
      public void DrawHeader(
        ...
        IList elementList)
      {
        EditorGUI.LabelField(headerRect, GUIContentHelper.TempContent(element != null ? "Serialized Property" : "IList"));
      }
      ...
      public void DrawElement(
        ...
        bool draggable)
      {
        EditorGUI.LabelField(rect, GUIContentHelper.TempContent(element != null ? element.displayName : listItem.ToString()));
      }

      public void DrawNoneElement(Rect rect, bool draggable) => EditorGUI.LabelField(rect, BetterReorderableList.Defaults.s_ListIsEmpty);
    }
  }
}
